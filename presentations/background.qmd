...
combined_data_selected <- combined_data %>%
  select(Player, year, rookie_ppg = `PTS...26.x`, career_ppg = `PTS...26.y`,
         ...
         rookie_rpg = `TRB...27.x`, career_rpg = `TRB...27.y`, rookie_spg = `STL...29.x`, career_spg = `STL...29.y`, rookie_bpg = `BLK...30.x`, career_bpg = `BLK...30.y`)
```
...
```{r}
baseline_correlations <- combined_data_selected |> select("rookie_ppg","career_ppg","rookie_apg","career_apg", "rookie_rpg","career_rpg", "rookie_spg", "career_spg","rookie_bpg","career_bpg") |>
...
```{r}
offense_cors <- combined_data_selected |> select("rookie_ppg","career_ppg","rookie_apg","career_apg", "rookie_rpg","career_rpg") |>
  cor(use="complete.obs")
...
career_cors <- combined_data_selected |> select("rookie_ppg","rookie_apg", "rookie_rpg","rookie_spg", "rookie_bpg","career_length") |>
  cor(use="complete.obs")
...
```{r}
rookie_p36_cors <- per_36 |> select("rookie_pp36","rookie_ap36", "rookie_rp36","rookie_sp36", "rookie_bp36","ccareer_length") |>
  cor(use="complete.obs")
...
cluster_vars <- combined_data_renamed |>
  select(age_rookie, years_career, rookie_pp36,career_pp36,rookie_ap36, career_ap36, rookie_rp36, career_rp36,rookie_sp36, career_sp36, rookie_bp36, career_bp36, rookie_top36, career_top36) |>
  na.omit()
```
...
```
Lots of bright color up in the top left. Seems like taking young guys actually works out for teams. 
...
I think that 5+ years in the league is good, but also some other stats are important. I value a bench player just as much as anyone, so I'm going to convert any stats I use from "regular" to per 36 minutes. But first, do minutes played in your rookie year have a correlation to your future? How frequent is it to find a player who didn't have big minutes early on who still succeeded? It'll be a similar process to our PPG comparison from last time. 
