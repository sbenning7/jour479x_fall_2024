...
```{r}
players <- read_csv("https://dwillis.github.io/sports-data-files/players24.csv")
...
players |>
  ...
  mutate(fga_g = FGA/G) |>
  arrange(desc(trueshooting)) |>
  filter(MP>100,fga_g>=3.0)
```

**ANSWER** Who do you get as the first result, and what was his trueshooting percentage?
TJ NeSmith from Radford, with a TS of 78.52%. He only played 

**ANSWER/REEVALUATION #2** I felt that some of the players still had results based off of a small sample size - some guys weren't shooting a lot per game, but were hitting everything...I would guess still a lot of guys who are just getting quick and easy buckets at unimportant times. I would've filtered just by total FGA, but I wanted to account for players who may have been injured who were still shooting well during their time, so I made a new variable, field goal attemps/game, and then included the filter so that it only showed guys who would take at least 3 shots per game -- seems reasonable in my eyes. There was some shifting in the top 10 true 'shooters'. 
...
Although the Lady Terps had 9 less shot attempts at the same time last year, they were nearly equal when it came to putting them on goal. The percentage of sog was higher last year. In my opinion, there isn't a 'better' year, because last season they were 'converting' more often, but this year they are getting more shots on goal. 